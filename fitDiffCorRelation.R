...
getFlmNetwork <- function(corr_list, fit_lm_path, sp_fit_pic_path) {
  ...
  p <- ggplot(fit_data) +
    ...
    labs(x = "Distance", y = "r_values", title = "r_values vs Distance") +
    geom_abline(aes(intercept = coef(fit_lm)[1], slope = coef(fit_lm)[2]), color = "red")
  ggsave(fit_lm_path, p, width = 10, height = 10, units = "in", dpi = 300)
  ...
  # print(sp_pic)
  ggsave(sp_fit_pic_path, sp_pic, width = 10, height = 10, units = "in", dpi = 300)
  print(count)
...
if(group_by == "bcr"){
  ...
  if (!file.exists(file.path(sub_dir, "BCR_inner_cor_p.RData"))) next
  load(file = file.path(sub_dir,"BCR_inner_cor_p.RData"))
  BCR_inner_cor_p %>%
    purrr::map2(.,names(.),function (AOU_corr_list, AOU){
      purrr::map2(AOU_corr_list, names(AOU_corr_list), function (AOU_corr_BCR_list, BCR){
        ...
        getFlmNetwork(AOU_corr_BCR_list,
                  fit_lm_path = file.path(bcr_fit_dir, paste0(AOU,"_", BCR,".png")),
                  sp_fit_pic_path = file.path(bcr_fit_sp_dir, paste0(AOU,"_", BCR,".png")))
      })})
}
}else {
  for(sub_dir in sub_dirs){
  ...
  pairwise_AOU_corr_pearson%>%
    purrr::map2(.,names(.),function (AOU_corr_list, AOU){
        getFlmNetwork(AOU_corr_list,
                  fit_lm_path = file.path(fit_dir, paste0(AOU,".png")),
                  sp_fit_pic_path = file.path(fit_sp_dir, paste0(AOU,".png")))
      })
}
}